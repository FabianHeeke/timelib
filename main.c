...
#include <stdio.h>
#include <stdlib.h>
const int GREGORIAN_CALENDAR_YEAR = 1582; //different calendar before this date

/**
 * checks if a given year is a leapyear
 * (returns 1 if leapyear, 0 if not, -1 if year is below)
 **/
...
int exist_date(int day, int month, int year){
    ...
    if ( (year >= 1582 && year <= 2400) && (month >= 1 && month <= 12) && day > 0){
